document.addEventListener('DOMContentLoaded', () => {
  ...
  btn.addEventListener("click", () => {
    ...
    // Category templates (note the commas — no syntax errors)
    const templates = {
      fruit: [
    ...
    // General poems stored separately (they use \n -> convert to <br> before output)
    const generalPoems = [
    ...
    // Build a candidate list: category templates + optionally some general poems
    const candidates = templates[category].slice();

    // Add some general poems for variety (50% chance)
    if (Math.random() < 0.5) {
      candidates.push(...generalPoems.map(p => p.replace(/\n/g, "<br>")));
    }
